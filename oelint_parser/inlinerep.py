...
def bb_utils_contains(_in):
    m = RegexRpl.match(r"(.*)bb\.utils\.contains\(.*?,\s*.*?,\s*(.*?),\s*.*?,\s*.\)", _in)
    if m:
        return m.group(1) + m.group(2).strip("\"'")
    return None

def bb_utils_contains_any(_in):
    m = RegexRpl.match(r"(.*)bb\.utils\.contains_any\(.*?,\s*.*?,\s*(.*?),\s*.*?,\s*.\)", _in)
    if m:
        return m.group(1) + m.group(2).strip("\"'")
    return None

def oe_utils_conditional(_in):
    m = RegexRpl.match(r"(.*)oe\.utils\.conditional\(.*?,\s*.*?,\s*(.*?),\s*.*?,\s*.\)", _in)
    if m:
        return m.group(1) + m.group(2).strip("\"'")
    return None
